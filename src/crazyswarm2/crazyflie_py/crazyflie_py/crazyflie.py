...
from tf2_ros import Buffer as TfBuffer, TransformListener as TfTransformListener, TransformException
...
class Crazyflie:
    ...
    def __init__(self, node, cfname, paramTypeDict, tf_buffer):
        ...
        self.armService = node.create_client(Arm, prefix + '/arm')
    ...
    def setGroupMask(self, groupMask):
        ...
        """
        # Note that this requires a recent firmware; older firmware versions not have such a parameter.
        try:
    ...
    def position(self):
        ...
        """     
        try:
            trans = self.tf_buffer.lookup_transform('world', f'cf{self.cfname}', rclpy.time.Time(),
    ...
    def setParams(self, params):
        ...
        for name, value in params.items():
            param_name = self.prefix[1:] + '.params.' + name
            param_type = self.paramTypeDict.get(name)

            if param_type is None:
                self.node.get_logger().warn(f'setParams: parameter type for {name} not found')
                continue
            if param_type == ParameterType.PARAMETER_INTEGER:
                param_value = ParameterValue(type=param_type, integer_value=int(value))
            elif param_type == ParameterType.PARAMETER_DOUBLE:
                ...
                self.node.get_logger().warn(f'setParams: unsupported parameter type for {name}')
                continue
    ...
    def setLEDColor(self, r, g, b):
        ...
        try:
            # Ensure 'ring/effect' is 7 (solid color); set if not
            effect = self.getParam('ring/effect')
            if effect != 7:
                self.setParam('ring/effect', 7)
            # Convert [0,1] to [0,255] and set params in batch
            self.setParams({
                'ring/solidRed': int(r * 255),
                'ring/solidGreen': int(g * 255),
                'ring/solidBlue': int(b * 255)
            })
        except Exception as e:
            self.node.get_logger().error(f"setLEDColor failed: {str(e)}")
