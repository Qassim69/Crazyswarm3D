    ...
    def run(self, raw):
        ...
        if self.x_d2.positiveZeroCrossing():
            try:
                ...
                self.node.get_logger().debug(f"Position types: {[type(p) for p in pos]}")
                self.candidatePoint = Point(x=float(pos[0]), y=float(pos[1]), z=float(pos[2]))
...
def sensorCallback(msg, pkg):
    ...
    # Extract the Gas Conc. Message from Sensor_Reading Topic[GasSensor]
    gas_conc = msg.raw
    # Apply Bout Detection logic on Gas Conc. Message[gas_conc]
    boutDetector.run(gas_conc)
    # Subscribe to /tf Topic for the Drone's exact [x, y, z] coordinates using Crazyflie's position() Function
    drone_pos = cf.position()
    # Log each Sensor Reading with ROS Time[seconds] as Timestamp
...
class MotionController:
    ...
    def __init__(self, node, bounds, max_velocity, max_time, service_name, cf, cfid):
        ...
        # This is a Client Service that Requests the Attraction and Repulsion Forces from APF_1_1_env.py [GetForcesServer class]
        ...
        # ROS Time Timestamp of when the Initial Mode[Setpoint] started
    ...
    def visualizeMode(self):
        ...
        # Latest Marker with the exact ROS Time Timestamp
...
# 4. Initialize the MotionController Object
...
# 5. Instruct the Crazyflie to Takeoff to the Desired Height.
...
# Verify TF Connection
    try:
       trans = tf_buffer.lookup_transform("map", cfname, rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=5))
